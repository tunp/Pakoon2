  ...
  for(int nWheel = 0; nWheel < m_nWheels; ++nWheel) {
    ...
    if(pWheel->m_style != BWheel::TWheelStyle::OFFROAD) {
      ...
      if(pWheel->m_style == BWheel::TWheelStyle::MOMSNEWWHEELS ||
        ...
        // rim arc
        int nSegs = 41;
        double dRimRadius1 = dHubRadius - 0.012 - 0.03;
        double dRimRadius2 = dHubRadius - 0.012;
        double dSlope = (dRimRadius2 - dRimRadius1) / 3.0;
        double dRimDepth = 0.0025;
        // outside edge
        glBegin(GL_TRIANGLE_STRIP);
        ...
        glEnd();
        // disc outer edge
        OpenGLHelpers::SetColorFull(0.6, 0.6, 0.6, 1);
        vDiscNormalCenter = BVector(-dDiscDepth - 0.5 * dDiscThickness, 0.0, 0.0);
        glBegin(GL_TRIANGLE_STRIP);
        {for (int nDiscSeg = 0; nDiscSeg <= nDiscSegs; ++nDiscSeg) {
          double dDiscSegAngle = (double(nDiscSeg) / double(nDiscSegs)) * 2.0 * g_cdPI;
          v = vHubCenter + BVector(0.0 - dDiscDepth, cos(dDiscSegAngle) * dDiscRadius1, sin(dDiscSegAngle) * dDiscRadius2);
          vNormal = v - vDiscNormalCenter;
          vNormal.ToUnitLength();
          glNormal3f(vNormal.m_dX, vNormal.m_dY, vNormal.m_dZ);
          glVertex3f(v.m_dX, v.m_dY, v.m_dZ);
          v = vHubCenter + BVector(-dDiscDepth - dDiscThickness, cos(dDiscSegAngle) * dDiscRadius2, sin(dDiscSegAngle) * dDiscRadius2);
          vNormal = v - vDiscNormalCenter;
          vNormal.ToUnitLength();
          glNormal3f(vNormal.m_dX, vNormal.m_dY, vNormal.m_dZ);
          glVertex3f(v.m_dX, v.m_dY, v.m_dZ);
